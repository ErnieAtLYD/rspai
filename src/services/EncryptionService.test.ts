...
describe("EncryptionService", () => {
	...
	describe("Web Crypto API Availability Check", () => {
		...
		it("should return false when crypto.subtle is undefined", () => {
			...
			delete (global.crypto as unknown as Record<string, unknown>).subtle;

			expect(encryptionService["isWebCryptoAvailable"]()).toBe(false);

			(global.crypto as unknown as Record<string, unknown>).subtle = originalSubtle;
		});
		...
		it("should return false when crypto.getRandomValues is undefined", () => {
			...
			delete (global.crypto as unknown as Record<string, unknown>).getRandomValues;
	...
	describe("Encryption", () => {
		...
		it("should throw error when Web Crypto API is not available", async () => {
			...
			await expect(
				uninitializedService.encrypt("data", "password")
			).rejects.toThrow(
		...
		it("should throw error when data is empty", async () => {
			...
			).rejects.toThrow("Data and password are required for encryption");
		});
	...
	describe("Decryption", () => {
		...
		it("should throw error when Web Crypto API is not available", async () => {
			...
			await expect(
				uninitializedService.decrypt(encryptedData, "password")
			).rejects.toThrow(
				"EncryptionService is not ready. Call initialize() first."
			);
		...
		it("should throw error when encrypted data is null", async () => {
			...
			await expect(
				encryptionService.decrypt(null as unknown as EncryptedData, "password")
			).rejects.toThrow(
	...
	describe("Data Validation", () => {
		it("should identify valid encrypted data", () => {
			...
			expect(encryptionService.isEncrypted(validEncryptedData)).toBe(
				true
			);
		});
	...
	describe("Password Validation", () => {
		...
		it("should reject passwords without uppercase letters", () => {
			const result =
				encryptionService.validatePassword("testpassword123");
			expect(result.valid).toBe(false);
			expect(result.message).toBe(
	...
	describe("Base64 Conversion", () => {
		...
		it("should handle empty data", () => {
			...
			const base64 =
				encryptionService["arrayBufferToBase64"](emptyBuffer);
			expect(base64).toBe("");

			const backToBuffer =
				encryptionService["base64ToArrayBuffer"](base64);
	...
	describe("Test Encryption Functionality", () => {
		it("should test encryption/decryption successfully", async () => {
			...
			mockCrypto.subtle.encrypt.mockResolvedValue(mockEncryptedBuffer);
			mockCrypto.subtle.decrypt.mockResolvedValue(
				new TextEncoder().encode("test-encryption-" + Date.now())
			);
	...
	describe("Integration Tests", () => {
		it("should perform full encrypt/decrypt cycle with real Web Crypto API", async () => {
			...
			if (realCrypto) {
				...
				const realService = new EncryptionService(
					app,
					{},
					mockErrorHandler
				);
				...
				const decrypted = await realService.decrypt(
					encrypted,
					password
				);
	...
	describe("Error Handling Edge Cases", () => {
		it("should handle malformed base64 data", () => {
			...
			const result =
				encryptionService["base64ToArrayBuffer"]("invalid-base64!@#");
		...
		it("should handle encryption with wrong data types", async () => {
			...
			await expect(
				encryptionService.encrypt(null as unknown as string, "password")
			).rejects.toThrow();
			await expect(
				encryptionService.encrypt(undefined as unknown as string, "password")
			).rejects.toThrow();
			await expect(
				encryptionService.encrypt("data", null as unknown as string)
			).rejects.toThrow();
			await expect(
				encryptionService.encrypt("data", undefined as unknown as string)
			).rejects.toThrow();
		});
		...
		it("should handle decryption with malformed encrypted data", async () => {
			...
			await expect(
				encryptionService.decrypt(malformedData, "password")
			).rejects.toThrow();
		});
	});
});
