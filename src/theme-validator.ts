/**
 * @module Theme Validator
 * @description Provides validation and quality assessment for themes, clusters, and topic models
...
export class CoreThemeValidator implements ThemeValidator {
	validateTheme(
		...
		if (theme.confidence < 0.5) {
			issues.push({
				type: "warning",
				category: "quality",
				message: `Theme confidence (${theme.confidence.toFixed(
					2
				)}) is below recommended threshold (0.5)`,
				severity: "medium",
				suggestion:
					"Consider gathering more evidence or refining theme keywords",
			});
			qualityScore -= 0.2;
		}
		...
		if (theme.frequency < 3) {
			issues.push({
				...
				severity: "low",
				suggestion:
					"Verify this represents a genuine pattern rather than noise",
			});
			qualityScore -= 0.1;
		}
		...
		if (theme.coherence < 0.4) {
			issues.push({
				type: "warning",
				category: "coherence",
				message: `Theme coherence (${theme.coherence.toFixed(
					2
				)}) is below recommended threshold (0.4)`,
				severity: "high",
				suggestion:
					"Review and refine theme keywords for better coherence",
			});
			qualityScore -= 0.3;
		}
		...
		if (theme.keywords.length < 3) {
			issues.push({
				...
				severity: "medium",
				suggestion:
					"Consider expanding keyword set or merging with similar themes",
			});
			qualityScore -= 0.15;
		}

		// Check evidence quality
		const lowQualityEvidence = theme.evidence.filter(
			(e) => e.relevanceScore < 0.5
		).length;
		if (lowQualityEvidence > theme.evidence.length * 0.5) {
			issues.push({
				...
				category: "quality",
				message:
					"More than half of the evidence has low relevance scores",
				severity: "medium",
  ...
  /**
   * Validate a cluster of documents
   * @param cluster - The cluster to validate
   * @param documents - The documents to validate the cluster against
   ...
   */
	validateCluster(
		...
		const clusterDocs = documents.filter((doc) =>
			cluster.documents.includes(doc.id)
		);
		...
		if (cluster.size < 3) {
			issues.push({
				...
				severity: "medium",
				suggestion:
					"Consider merging with similar clusters or requiring larger minimum cluster size",
			});
		}
		...
		if (cluster.coherence < 0.3) {
			issues.push({
				type: "error",
				category: "coherence",
				message: `Cluster coherence (${cluster.coherence.toFixed(
					2
				)}) is below minimum threshold (0.3)`,
				severity: "high",
				suggestion:
					"Recalculate cluster or adjust clustering parameters",
			});
		}
		...
		if (cluster.silhouetteScore < 0.2) {
			issues.push({
				type: "warning",
				category: "quality",
				message: `Cluster silhouette score (${cluster.silhouetteScore.toFixed(
					2
				)}) indicates poor separation`,
				severity: "medium",
				suggestion:
					"Review clustering algorithm parameters or document preprocessing",
			});
		}

		// Validate document availability
		const missingDocs = cluster.documents.filter(
			(id) => !clusterDocs.find((doc) => doc.id === id)
		);
		if (missingDocs.length > 0) {
			issues.push({
				...
				severity: "high",
				suggestion:
					"Update cluster document references or provide missing documents",
			});
		}
		...
		const qualityMetrics: ClusterQualityMetrics = {
			...
			separation:
				cluster.interClusterDistance - cluster.intraClusterDistance,
		};
	...
	validateTopicModel(
		...
		if (model.coherence < 0.3) {
			issues.push({
				type: "warning",
				category: "coherence",
				message: `Topic coherence (${model.coherence.toFixed(
					2
				)}) is below recommended threshold (0.3)`,
				severity: "high",
				suggestion:
					"Adjust topic modeling parameters or increase training data",
			});
		}
		...
		if (model.words.length < 5) {
			issues.push({
				...
				severity: "medium",
				suggestion:
					"Increase minimum words per topic or review topic extraction",
			});
		}

		// Check word probability distribution
		const totalProbability = model.words.reduce(
		...
		if (Math.abs(totalProbability - 1.0) > 0.1) {
			issues.push({
				type: "warning",
				category: "quality",
				message: `Word probabilities don't sum to 1.0 (sum: ${totalProbability.toFixed(
					2
				)})`,
				severity: "low",
		...
		if (topWordWeight > 0.8) {
			issues.push({
				...
				category: "quality",
				message:
					"Topic is dominated by a single word, may lack diversity",
				severity: "medium",
				suggestion:
					"Review topic modeling parameters to encourage word diversity",
			});
		}

		// Check document associations
		const highProbDocs = model.documents.filter(
			(d) => d.probability > 0.5
		).length;
		if (highProbDocs === 0) {
			issues.push({
				...
				severity: "medium",
				suggestion:
					"Review topic-document associations or topic quality",
			});
		}
	...
	private generateThemeSuggestions(
		theme: DetectedTheme,
		...
		if (theme.confidence < 0.7) {
			suggestions.push(
				"Consider gathering more supporting evidence to increase confidence"
			);
		}

		if (theme.keywords.length > 20) {
			suggestions.push(
				"Theme may be too broad - consider splitting into sub-themes"
			);
		}

		if (theme.frequency < 5) {
			suggestions.push(
				"Low frequency theme - verify it represents a meaningful pattern"
			);
		}

		if (issues.some((i) => i.category === "coherence")) {
			suggestions.push(
				"Review and refine keywords to improve theme coherence"
			);
		}

		if (theme.evidence.length > 50) {
			suggestions.push(
				"Large evidence set - consider sampling representative examples"
			);
		}
	...
	private generateClusterSuggestions(
		cluster: DocumentCluster,
		issues: ValidationIssue[]
	): string[] {
		const suggestions: string[] = [];
		...
		if (cluster.size < 5) {
			suggestions.push(
				"Small cluster - consider merging with similar clusters"
			);
		}

		if (cluster.coherence < 0.5) {
			suggestions.push(
				"Low coherence - review clustering parameters or document preprocessing"
			);
		}

		if (cluster.silhouetteScore < 0.3) {
			suggestions.push(
				"Poor cluster separation - adjust clustering algorithm or similarity threshold"
			);
		}

		if (issues.some((i) => i.type === "error")) {
			suggestions.push(
				"Address critical errors before using this cluster for theme detection"
			);
		}
	...
	private generateTopicSuggestions(
		...
		if (model.coherence < 0.5) {
			suggestions.push(
				"Low coherence - increase training iterations or adjust parameters"
			);
		}

		if (model.words.length < 8) {
			suggestions.push(
				"Few words in topic - consider increasing words per topic"
			);
		}

		if (model.documents.length === 0) {
			suggestions.push(
				"No associated documents - review topic-document assignment"
			);
		}

		const avgProbability =
			model.documents.reduce((sum, d) => sum + d.probability, 0) /
			model.documents.length;
		if (avgProbability < 0.3) {
			suggestions.push(
				"Low document-topic probabilities - review topic modeling quality"
			);
		}
	...
	validateThemeCollection(
		...
		const uniqueDocuments = new Set(
			themes.flatMap((t) => t.evidence.map((e) => e.documentId))
		).size;
		...
		if (coverage < 0.5) {
			collectionIssues.push({
				type: "warning",
				category: "quality",
				message: `Theme collection covers only ${(
					coverage * 100
				).toFixed(1)}% of documents`,
				severity: "medium",
				suggestion:
					"Consider adjusting detection parameters to improve coverage",
			});
		}

		// Calculate overall quality
		const validThemes = themeValidations.filter(
			(tv) => tv.validation.isValid
		).length;
		const avgQuality =
			themeValidations.reduce(
				(sum, tv) => sum + tv.validation.qualityScore,
				0
			) / themes.length;
		...
		if (themes.length > 50) {
			suggestions.push(
				"Large number of themes - consider increasing minimum frequency threshold"
			);
		}

		if (validThemes / themes.length < 0.8) {
			suggestions.push(
				"Many invalid themes detected - review detection parameters"
			);
		}
	...
	private findThemeOverlaps(
		themes: DetectedTheme[]
	): Array<{ themeA: string; themeB: string; similarity: number }> {
		...
		for (let i = 0; i < themes.length; i++) {
			for (let j = i + 1; j < themes.length; j++) {
				const similarity = this.calculateThemeOverlap(
					themes[i],
					themes[j]
				);
				if (similarity > 0.7) {
	...
	private calculateThemeOverlap(
		themeA: DetectedTheme,
		themeB: DetectedTheme
	): number {
		const keywordsA = new Set(
			themeA.keywords.map((k) => k.term.toLowerCase())
		);
		const keywordsB = new Set(
			themeB.keywords.map((k) => k.term.toLowerCase())
		);

		const intersection = new Set(
			[...keywordsA].filter((k) => keywordsB.has(k))
		);
