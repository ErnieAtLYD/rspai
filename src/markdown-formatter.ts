...
import { GeneratedSummary, Insight, TrendAnalysis, SummaryMetric } from './summary-generation-interfaces';
...
export class CoreMarkdownFormatter implements MarkdownFormatter {
	...
	async formatSummary(
		...
		try {
			...
			const elements = await this.generateMarkdownElements(summary, config);
			const content = this.renderElements(elements, config.preferences.styles || this.getDefaultStyles());
			...
			if (config.enableOptimization) {
				optimizedContent = this.optimizeContent(content, config.preferences);
			}
			
			// Generate metadata
			const metadata = this.generateMetadata(summary, config, wordCount, readingTime, startTime);
			...
			const result: MarkdownGenerationResult = {
				...
				cacheHit: false,
				availableFormats: ['markdown', 'html', 'pdf', 'docx', 'plain-text', 'json']
			};
			...
			this.logger.info(`Generated markdown summary in ${result.generationTime}ms`);
			return result;
	...
	async formatInsights(
		...
		if (config.preferences.useTables && insights.length > 3) {
			...
			const items = insights.map(insight => {
				...
				if (config.preferences.includeEvidence && insight.evidence.length > 0) {
	...
	async formatRecommendations(
		...
		if (recommendations.length > 5) {
			...
			for (const [urgency, recs] of Object.entries(grouped)) {
				...
				sections.push(this.formatHeading(`${urgency.charAt(0).toUpperCase() + urgency.slice(1)} Priority`, 3, styles));
			...
			const items = recommendations.map(rec => {
				let item = `**${rec.title}** - ${rec.description}`;
				if (config.preferences.useCallouts && rec.urgency === 'urgent') {
					return this.formatCallout(item, 'warning', 'Urgent Action Required');
				}
				return item;
			});
			sections.push(this.formatList(items, 'ordered', styles));
	...
	async formatTrends(
		...
		for (const trend of trends) {
			...
			if (config.preferences.detailLevel !== 'minimal' && trend.dataPoints.length > 0) {
				const items = trend.dataPoints.map(point => 
					`${point.label}: ${point.value} (${point.date.toLocaleDateString()})`
				);
				sections.push(this.formatList(items, 'unordered', styles));
			}
		}
	...
	async formatMetrics(
		...
		if (config.preferences.useTables) {
			...
			const rows = metrics.map(metric => [
				...
				metric.change ? `${metric.change > 0 ? '+' : ''}${metric.change}%` : 'N/A'
			]);
			sections.push(this.formatTable(headers, rows, styles));
		} else {
			const items = metrics.map(metric => {
				...
				if (metric.change !== undefined) {
					const changeIcon = metric.change > 0 ? '📈' : metric.change < 0 ? '📉' : '➡️';
					item += ` ${changeIcon} ${metric.change > 0 ? '+' : ''}${metric.change}%`;
				}
				return item;
			});
			sections.push(this.formatList(items, 'unordered', styles));
	...
	formatHeading(text: string, level: HeadingLevel, styles: MarkdownStyles): string {
	...
	formatList(items: string[], type: ListType, styles: MarkdownStyles): string {
		...
		for (let i = 0; i < items.length; i++) {
			...
			switch (type) {
				case 'ordered':
					prefix = styles.orderedListStyle.replace('1', (i + 1).toString());
					break;
				case 'unordered':
	...
	formatTable(headers: string[], rows: string[][], styles: MarkdownStyles): string {
	...
	formatCallout(content: string, type: CalloutType, title?: string): string {
		...
		switch (this.preferences.styles?.calloutStyle || 'obsidian') {
			...
			case 'custom':
				const prefix = this.preferences.styles?.customCalloutPrefix || '>';
	...
	async applyTemplate(content: string, template: MarkdownTemplate): Promise<string> {
	...
	validateTemplate(template: MarkdownTemplate): ValidationResult {
		...
		if (template.sections.length === 0) {
			warnings.push({
				...
				suggestion: 'Add at least one section to make the template useful'
			});
		}
	...
	calculateComplexity(content: string): number {
		...
		const structureScore = (headings + lists + tables) / content.length * 1000;
	...
	validateMarkdown(content: string): ValidationResult {
		...
		if (headings.length === 0) {
			warnings.push({
				...
				suggestion: 'Add headings to improve document structure and accessibility'
			});
		}
	...
	optimizeContent(content: string, preferences: MarkdownFormattingPreferences): string {
		...
		if (preferences.bulletStyle) {
			optimized = optimized.replace(/^[\s]*[*+]\s/gm, `${preferences.bulletStyle} `);
		}
		...
		if (preferences.styles?.emphasisStyle) {
			...
			optimized = optimized.replace(new RegExp(`${otherStyle}([^${otherStyle}]+)${otherStyle}`, 'g'), `${style}$1${style}`);
		}
		
		// Add reading time if requested
		if (preferences.includeReadingTime) {
			const readingTime = this.calculateReadingTime(optimized);
			optimized = `*Reading time: ${readingTime} minute${readingTime !== 1 ? 's' : ''}*\n\n${optimized}`;
		}
	...
	updatePreferences(preferences: Partial<MarkdownFormattingPreferences>): void {
	...
	private async generateMarkdownElements(
		...
		// Executive summary
		if (prefs.includedSections.includes('executive-summary') && summary.executiveSummary) {
			if (prefs.useHeadings) {
				elements.push({
					type: 'heading',
		...
		// Insights
		if (prefs.includedSections.includes('key-insights') && summary.insights.length > 0) {
			const insightContent = await this.formatInsights(summary.insights, config);
			elements.push({
				type: 'paragraph',
		...
		// Recommendations
		if (prefs.includedSections.includes('recommendations') && summary.recommendations.length > 0) {
			...
			const recContent = await this.formatRecommendations(generatedRecs, config);
			elements.push({
				type: 'paragraph',
				content: recContent
			});
		...
		// Trends
		if (prefs.includedSections.includes('trends') && summary.trends.length > 0) {
			const trendContent = await this.formatTrends(summary.trends, config);
			elements.push({
				type: 'paragraph',
		...
		// Metrics
		if (prefs.includedSections.includes('metrics') && summary.keyMetrics.length > 0) {
			const metricContent = await this.formatMetrics(summary.keyMetrics, config);
			elements.push({
				type: 'paragraph',
	...
	private renderElements(elements: MarkdownElement[], styles: MarkdownStyles): string {
		const sections: string[] = [];
		...
		for (const element of elements) {
			switch (element.type) {
				case 'heading':
					...
					sections.push(this.formatHeading(element.content, heading.level, styles));
					break;
				case 'paragraph':
				...
				case 'list':
					...
					sections.push(this.formatList(items, list.listType, styles));
					break;
				case 'table':
					const table = element as MarkdownTable;
					sections.push(this.formatTable(table.headers, table.rows, styles));
					break;
				case 'callout':
					const callout = element as MarkdownCallout;
					sections.push(this.formatCallout(element.content, callout.calloutType, callout.title));
					break;
	...
	private generateMetadata(
		...
		return {
			...
			evidenceCount: summary.insights.reduce((sum, insight) => sum + insight.evidence.length, 0),
			sourceFiles: [], // Not available in current interface
			dateRange: summary.dateRange || { start: new Date(), end: new Date() },
	...
	private generateCacheKey(summary: GeneratedSummary, config: MarkdownGenerationConfig): string {
	...
	private groupRecommendationsByUrgency(recommendations: GeneratedRecommendation[]): Record<string, GeneratedRecommendation[]> {
	...
	private exportToMarkdown(content: string, options: ExportOptions): ExportResult {
	...
	private exportToHTML(content: string, options: ExportOptions): ExportResult {
	...
	private exportToPlainText(content: string, options: ExportOptions): ExportResult {
	...
	private exportToJSON(content: string, options: ExportOptions): ExportResult {
		const jsonData = {
			content,
	...
	private mergeWithDefaults(preferences: Partial<MarkdownFormattingPreferences>): MarkdownFormattingPreferences {
		return {
			useHeadings: true,
			maxHeadingLevel: 6,
			...
			bulletStyle: '-',
			numberedListStyle: '1.',
			includedSections: ['executive-summary', 'key-insights', 'recommendations', 'trends', 'metrics'],
			sectionOrder: ['executive-summary', 'key-insights', 'recommendations', 'trends', 'metrics'],
			customSections: [],
