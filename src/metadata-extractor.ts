...
export class MetadataExtractor {
	...
	async extractMetadata(
		...
		try {
			...
			if (
				!privacyStatus.canProcess &&
				this.config.respectPrivacyFilters
			) {
				return this.createMinimalMetadata(
					filePath,
					content,
					privacyStatus,
					startTime
			...
			const obsidianFeatures = this.obsidianHandler.extractFeatures(
				content,
				filePath
			);
			...
			const metadata: NormalizedMetadata = {
				...
				// Content metadata
				frontmatter:
					this.extractAndNormalizeFrontmatter(parsedDocument),
				tags: this.extractAndNormalizeTags(
					parsedDocument,
					obsidianFeatures
				),
				links: this.extractAndNormalizeLinks(
					parsedDocument,
					obsidianFeatures,
					content
				),
				...
				// Obsidian-specific
				obsidianFeatures:
					this.summarizeObsidianFeatures(obsidianFeatures),
			...
			this.logger.error(
				`Failed to extract metadata for ${filePath}`,
				error
			);
			throw error;
		}
	...
	async extractBatchMetadata(
		...
		this.logger.info(
			`Starting batch metadata extraction for ${files.length} files`
		);
		...
		for (const file of files) {
			try {
				const fileMetadata = await this.extractMetadata(
					file.path,
					file.content,
					file.stats
				);
	 ...
	 */
	private extractAndNormalizeFrontmatter(
	...
	private extractAndNormalizeLinks(
		...
		const linkFeatures = obsidianFeatures.filter(
			(f) => f.type === "wikilink" || f.type === "embed"
		);
	...
	private buildReferenceGraph(
		...
		for (const metadata of metadataList) {
			const node: GraphNode = {
				...
				title:
					metadata.frontmatter.normalized.title || metadata.fileName,
				type: "note",
				metadata: {
	...
	private enrichMetadataWithReferences(
		...
		for (const metadata of metadataList) {
			...
			metadata.references = [
				...outgoing.map((edge) =>
					this.createReference(edge, "outgoing")
				),
				...incoming.map((edge) =>
					this.createReference(edge, "incoming")
				),
			];
		}
	}
	...
	private summarizeObsidianFeatures(
		features: ObsidianFeature[]
	): ObsidianFeatureSummary {
		...
		return {
			...
			hasDataview:
				"dataview-query" in featureTypes ||
				"dataview-inline" in featureTypes,
			hasMermaid: "mermaid-diagram" in featureTypes,
			hasCustomSyntax: features.some(
				(f) => f.plugin && f.plugin !== "core"
			),
			features,
		};
	}
	...
	private extractFileType(target: string): string | undefined {
		...
		return lastDot > 0
			? target.substring(lastDot + 1).toLowerCase()
			: undefined;
	}
