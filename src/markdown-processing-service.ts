...
import { MetadataExtractor, NormalizedMetadata, ReferenceGraph } from './metadata-extractor';
...
export interface ProcessingError {
  ...
  message: string;
  filePath?: string;
  component: 'parser' | 'metadata' | 'privacy' | 'features' | 'sections' | 'service';
...
export class MarkdownProcessingService {
  ...
  private initializeComponents(): void {
    try {
      ...
      this.logger.info('Markdown processing service initialized successfully');
    } catch (error) {
      this.logger.error('Failed to initialize markdown processing service', error);
      throw error;
    }
  }
  ...
  async processFile(filePath: string): Promise<ProcessingResult> {
    ...
    try {
      ...
      if (this.config.enableMetadataExtraction) {
        try {
          result.metadata = await this.metadataExtractor.extractMetadata(
            filePath,
            content,
            {
              size: file.stat.size,
      ...
      if (this.config.enableSectionDetection) {
        try {
          result.sections = await this.detectSections(parseResult.data, content);
        } catch (error) {
          result.errors.push({
      ...
      this.logger.debug(`Successfully processed ${filePath} in ${result.processingTime}ms`);
      return result;
  ...
  async processBatch(filePaths: string[]): Promise<BatchProcessingResult> {
    ...
    this.logger.info(`Starting batch processing of ${filePaths.length} files`);
    ...
    for (let i = 0; i < filePaths.length; i += this.config.batchSize) {
      ...
      this.logger.debug(`Processed batch ${Math.floor(i / this.config.batchSize) + 1}/${Math.ceil(filePaths.length / this.config.batchSize)}`);
    }

    // Build reference graph if enabled
    let referenceGraph: ReferenceGraph | undefined;
    if (this.config.enableReferenceGraph && this.config.enableMetadataExtraction) {
      try {
        ...
        if (metadataList.length > 0) {
          const batchResult = await this.metadataExtractor.extractBatchMetadata(
            metadataList.map(m => ({
              path: m.filePath,
    ...
    this.logger.info(`Batch processing completed: ${successfulFiles}/${results.length} files processed successfully in ${totalProcessingTime}ms`);
   ...
   */
  private async detectSections(document: ParsedMarkdown, content: string): Promise<DocumentSection[]> {
    const sections: DocumentSection[] = [];
    const lines = content.split('\n');
    
    // Find all headings
    const headings = document.elements.filter(el => el.type === 'heading');
    ...
    for (let i = 0; i < headings.length; i++) {
      ...
      const endLine = nextHeading ? nextHeading.position.startLine - 1 : lines.length - 1;
      ...
      const section: DocumentSection = {
        ...
        position: {
          ...
          endChar: nextHeading ? nextHeading.position.startChar - 1 : content.length
        },
        subsections: [],
        metadata: {
          ...
          hasObsidianFeatures: /!\[\[.*?\]\]|> \[!/.test(sectionContent),
  ...
  private categorizeSection(heading: any, content: string): SectionCategory {
    ...
    if (title.includes('analysis') || title.includes('analyze')) return 'analysis';
    if (title.includes('conclusion') || title.includes('conclude')) return 'conclusion';
    if (title.includes('method') || title.includes('approach')) return 'methodology';
    if (title.includes('result') || title.includes('finding')) return 'results';
    if (title.includes('discussion') || title.includes('discuss')) return 'discussion';
    if (title.includes('reference') || title.includes('citation')) return 'references';
    if (title.includes('appendix') || title.includes('supplement')) return 'appendix';
    if (title.includes('todo') || title.includes('task')) return 'todo';
    if (title.includes('note') || title.includes('observation')) return 'notes';
    
    // Check content patterns
    if (content.includes('```') && content.split('```').length > 2) return 'code';
