...
export class DefaultAIModelFactory implements AIModelFactory {
	...
	async createModel(config: AIModelConfig): Promise<AIModelAdapter> {
		try {
			...
			this.logger.info(
				`Created AI model adapter: ${config.name} (${adapterType})`
			);
			return adapter;
		} catch (error) {
	...
	async validateConfig(
		...
		this.logger.debug("Validating AI model configuration", {
			name: config.name,
			type: config.type,
		...
		if (config.type === "cloud") {
			const hasBuiltIn = this.hasBuiltInEndpoint(config);
			this.logger.debug("Cloud model validation", {
				hasEndpoint: !!config.endpoint,
				hasBuiltInEndpoint: hasBuiltIn,
				endpoint: config.endpoint,
			});

			if (!config.endpoint && !hasBuiltIn) {
				errors.push(
					"Cloud models require an endpoint or built-in service configuration"
				);
			}
		}
		...
		if (
			...
			(config.maxTokens <= 0 || config.maxTokens > 100000)
		) {
		...
		this.logger.debug("Configuration validation result", {
	...
	private determineAdapterType(config: AIModelConfig): string {
		// Use the name field to determine adapter type (this is set by the AI service)
		if (config.name) {
	...
	private hasBuiltInEndpoint(config: AIModelConfig): boolean {
		if (!config.name) {
			this.logger.debug("hasBuiltInEndpoint: no config.name provided");
			return false;
		}
		...
		const hasBuiltIn = builtInServices.includes(name);
		this.logger.debug("hasBuiltInEndpoint check", {
...
export function getAIModelFactory(logger?: Logger): DefaultAIModelFactory {
	if (!factoryInstance) {
		if (!logger) {
			throw new Error(
				"Logger is required for first factory initialization"
			);
		}
